...
-- spGetTeacherImpactDetail_US116 2,3,3,17,5,1,-1        
 ...
 BEGIN  
  ...
  SELECT s.StudentId,   
   ...
   @AssessmentType as AssessmentType  ,
    CASE(@ViewScaledScore) WHEN 1 THEN dbo.udfGetMeetExceedCriteriaValue(@DistrictId,@SubjectId,@SchoolYearId,@AssessmentTypeId,ass.ScaledScoreProjDif) 
    ELSE dbo.udfGetMeetExceedCriteriaValue(@DistrictId,@SubjectId,@SchoolYearId,@AssessmentTypeId, ass.ScoreProjDif) END AS MeetExceedValue 
