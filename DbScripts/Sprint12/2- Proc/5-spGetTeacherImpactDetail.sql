...
AS     
	...
	DECLARE @AssessmentCode  VARCHAR(20)      
			,@Query VARCHAR(MAX)
    SELECT  @AssessmentCode = AssessmentCode     
    ...
    BEGIN    
		SELECT 
			...
			(CASE(@AssessmentCode) WHEN 'F & P' THEN ass.GrowthCalc Else ass.DistrictPercentile END ) AS  DistrictPercentile ,       
			a.AssessmentId,    
			s.LastName,    
			@AssessmentCode AS AssessmentCode, 
